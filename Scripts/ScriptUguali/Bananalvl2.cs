...
public class Bananalvl2 : MonoBehaviour
{
    ...
    public void Spawn(Transform spawnPosition, Vector2 directionPosizione, float Time)
    {
        ...
        if (PlayerPrefs.GetInt("SaveSetting") == 1)
        {  //Se il player ha modificato la forza del lancio nel menu opzioni, allora caricale.
            ChangeValue();
    ...
    private void OnCollisionEnter(Collision other)
    {
        ...
        if (other.gameObject.tag == GameObject.Find("ManagerEquation").GetComponent<MANAGERLVL2>().confronta().ToString())

        {

            GameObject.Find("ManagerEquation").GetComponent<MANAGERLVL2>().incrementa(); //Evoca la funzione per scorrere di uno l'array delle  soluzioni


            GrabThatValue.instance.addPunteggio(); // Evoca la funzione per aggiungere +1 al punteggio


            GameObject.Find("ManagerEquation").GetComponent<MANAGERLVL2>().lancia(); //Evoca la funzione per scorrere di uno l'array delle equazioni
            GameObject.Find("Cartello2 Variant(Clone)").GetComponent<Cartello>().DeSpawn(); //Disattiva la vecchia eq in scena
