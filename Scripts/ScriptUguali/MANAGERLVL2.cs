...
public class MANAGERLVL2 : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        for (int i = 0; i < lvl1.Length; i++)
        {
            ...
            lvl1[i].GetComponent<LVLEQUAZIONI>().generate(); // inserisco l'eq generata randomicamente come oggetto figlio del cartello
            array[i] = lvl1[i].GetComponent<LVLEQUAZIONI>().getSoluzione(); // inserisco la soluzione dell'eq appena generata, in questo array 
